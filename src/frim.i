...
if (get_env("USER") == "eric") {
  write, "FIXME: check minimal variance in kolmogorov_covariance";
}
 ...
 *   Fried's parameter in pixels (default: R0=1).  Keyword SIX has the
 *   same meaning as in frim_lane (which see).
 *
 ...
 *   numberof(SF) = 2*(P + 1).  The structure function SF of a function
 *   F is defined as:
 *
 *        SF(r) = < (f(r' + r) - f(r'))^2 >
 *
 ...
 *   DIM = 2^P + 1.  See frim_gen_2d() for a description of the argument
 *   COV.
 *
 ...
 *   array of pseudo-random values with normal distribution (see
 *   random_n), the result is a random 2-D field with covariance
 *   (approximatively) equals to COV.  See kolmogorov_covariance as an
 *   example for computing COV.
 *
