% kolmogorov: Generation of a Kolmogorov phase screen.
%
% Synopsis:
%
%     p = kolmogorov(dim, r0=dim, orig=0);
%
...
%
function a = kolmogorov(dim, r0, orig)
    % Default arguments.
    if nargin < 2
        r0 = dim;
    end
    ...
    while h > 1
        ...
        a(j,j) = c3*a(j,j) + 0.25*(a(jp,jp) + a(jp,jn) + ...
                                   a(jn,jp) + a(jn,jn));

        if 2*s < n
            % Set the samples at the center of losanges.
            k = 1+s:s:n-s; % vertice indices
            kp = k - h;    % previous indices
            kn = k + h;    % next indices
            a(j,k) = c4*a(j,k) + 0.25*(a(j,kp) + a(j,kn) + a(jp,k) + a(jn,k));
            a(k,j) = c4*a(k,j) + 0.25*(a(k,jp) + a(k,jn) + a(kp,j) + a(kn,j));
        end
